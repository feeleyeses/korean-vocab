...
import {House,BookOpen,Layers3,Library as LibraryIcon,RotateCcw} from 'lucide-react';
...
const nav=[['home','首页',House],['study','学习',BookOpen],['poly','多义',Layers3],['library','词库',LibraryIcon],['review','复习',RotateCcw]];
...
export default function App(){
  ...
  function go(next){if(next==='study'){startLearning();return;}setSession(null);setView(next);}
  ...
  function startReview(mode='due',ids){if(mode.startsWith('poly-')){startPoly(mode);return;}const source=Array.isArray(ids)?words.filter(w=>ids.includes(w.id)):words;const queue=reviewQueue(source,store.memories,Array.isArray(ids)?'full':mode,store.favorites);setSession({id:crypto.randomUUID(),mode:'review',reviewMode:mode,title:mode==='sound'?'音变专项':'复习',queue,levels:[1,2,3,4,5,6]});setView('review');}
  ...
  return <div className="app-shell">
    ...
    <main className={`app-main view-${view}`}>
      ...
      {view==='review'&&(session?<><ReviewMenu words={words} store={store} startReview={startReview} active={session.reviewMode}/><Session key={session.id} session={session} words={words} store={store} onExit={()=>go('review')} onRestart={()=>startReview(session.reviewMode)}/></>:<ReviewHome words={words} store={store} startReview={startReview} startSound={()=>startReview('sound')}/>)}
    ...
    <nav className="mobile-nav" aria-label="移动端主导航">{nav.filter(([id])=>id!=='home').map(([id,label,Icon])=><button key={id} className={view===id?'active':''} onClick={()=>go(id)}><Icon size={20}/><span>{label}</span></button>)}</nav>
