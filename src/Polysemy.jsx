...
import { Button,WordHero,WordActions,EmptyState,SenseCard } from './components.jsx';
...
function PolyQuestion({item,index,total,mode,words,store,next}) {
  ...
  return <><article className="poly-question" data-word={item.word.headword} data-submitted={!!result}>
    <div className="poly-prompt"><h2>{mode==='poly-learn'?'学习新多义':multi?'复习·一词多义（多选）':'复习·一词多义（单选）'}</h2><WordHero word={item.word}/><p className="muted">{index+1} / {total} · {senseGroups(item.word).length} 个释义</p><WordActions word={item.word} store={store}/></div>
    <div className="poly-answer"><h2>{multi?'请选择这个词的所有正确释义':'请选择正确释义'}</h2><div className="poly-options" role="group" aria-label={multi?'多选释义':'单选释义'}>{options.map(o=>{
      ...
      return <label className={`poly-option ${isSelected?'selected':''} ${correct?'correct':''} ${wrong?'error':''}`} key={o.id}><input type={multi?'checkbox':'radio'} name="poly-answer" checked={isSelected} disabled={!!result} onChange={()=>setSelected(s=>multi?(s.includes(o.id)?s.filter(v=>v!==o.id):[...s,o.id]):[o.id])}/><span>{o.gloss}</span>{result&&correct&&<Check size={20}/>}</label>;
    })}</div><div className="poly-submit">{result?<><span className="recorded"><Check size={20}/>已记录到复习档案</span><Button variant="primary" onClick={next}>下一题<ArrowRight size={20}/></Button></>:<Button variant="primary" disabled={!selected.length} onClick={submit}>提交所选释义</Button>}</div>{result&&<p className="muted">本次选对 {result.chosen.length}/{result.correct.length} 个释义 · 漏选 {result.missed.length} · 误选 {result.wrong.length}</p>}</div>
  </article>{result&&<section className="poly-results">
    <h2>本词释义结果</h2>
    <p className="result-summary">{result.isCorrect?<><Check size={20}/>本题全对 · 本次选对 {result.chosen.length}</>:<>本次选对 {result.chosen.length} / 漏选 {result.missed.length} / 误选 {result.wrong.length}</>}</p>
