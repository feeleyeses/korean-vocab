...
import { Button,WordHero,WordActions,EmptyState,SenseCard,Progress } from './components.jsx';
...
function PolyQuestion({item,index,total,mode,words,store,next}) {
  ...
  return <><article className="poly-question" data-word={item.word.headword} data-submitted={!!result}>
    <div className="poly-prompt"><div className="poly-heading"><h2>{mode==='poly-learn'?'学习新多义':multi?'复习·一词多义（多选）':'复习·一词多义（单选）'}</h2><strong>{index+1} / {total}</strong></div><Progress value={index+1} max={total} label="多义题目进度"/><WordHero word={item.word}/><p className="muted poly-sense-count">{senseGroups(item.word).length} 个释义</p><WordActions word={item.word} store={store}/></div>
    ...
    <h2>本词释义结果</h2>
    <div className="result-summary" role="status"><span className="chosen"><CircleCheck size={20}/>{result.isCorrect?'本题全对':'选对'} <strong>{result.chosen.length}/{result.correct.length}</strong></span>{result.missed.length>0&&<span className="missed"><CircleAlert size={20}/>漏选 <strong>{result.missed.length}</strong></span>}{result.wrong.length>0&&<span className="wrong"><CircleX size={20}/>误选 <strong>{result.wrong.length}</strong></span>}</div>
