...
const scope=await read('./expanded-scope.json'),raw=await read('../../data/vocabulary.json'),base=await read('./publication-baseline.json'),tat=await read('./unblock-local/tatoeba-expanded-index.json');
if(hash(raw)!==scope.datasetHash)throw Error('Production drift');
if(tat.sourceAccess!=='available')throw Error('Tatoeba unavailable; coverage unknown');
const registry=(await read('./source-registry.json')).sources.Tatoeba;
...
await fs.writeFile(new URL('./expanded-results.json',import.meta.url),JSON.stringify({report,policy,candidates:ranked},null,2));
await fs.writeFile(new URL('./expanded-report.json',import.meta.url),JSON.stringify(report,null,2));
