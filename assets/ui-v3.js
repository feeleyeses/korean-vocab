...
function normalizeCard(){
  ...
  const mobile=matchMedia('(max-width:900px)').matches, cardH='500px', wordH=mobile?'132px':'136px', actionH=mobile?'50px':'52px', actionX=mobile?'14px':'24px';
  ...
  card.dataset.kwfUi='v3.2'; card.dataset.kwfState=revealed?'revealed':'initial'; card.dataset.kwfMode=review?'review':'learn';
  ...
  const pre=card.querySelector('.pre-answer'); if(pre){for(const [p,v] of [['position','relative'],['flex','1 1 0'],['height','auto'],['min-height','0'],['max-height','none']])important(pre,p,v);const grid=pre.querySelector(':scope>div');if(grid)for(const [p,v] of [['position','absolute'],['left',actionX],['right',actionX],['bottom','0'],['height',actionH],['display','grid'],['grid-template-columns','repeat(3,minmax(0,1fr))']])important(grid,p,v);}
  ...
  const cont=card.querySelector('.continue'); if(cont)for(const [p,v] of [['position','absolute'],['left',actionX],['right',actionX],['bottom',mobile?'14px':'18px'],['width','auto'],['height',actionH],['min-height',actionH],['max-height',actionH],['margin','0'],['z-index','30']])important(cont,p,v);
  const question=card.querySelector('.review-question'); if(question){for(const [p,v] of [['position','relative'],['flex','1 1 0'],['height','auto'],['min-height','0'],['max-height','none'],['overflow','visible']])important(question,p,v);const grid=question.querySelector(':scope>div');if(grid)for(const [p,v] of [['position','absolute'],['left',actionX],['right',actionX],['bottom','0'],['display','grid'],['grid-template-columns','repeat(2,minmax(0,1fr))'],['grid-template-rows',mobile?'repeat(2,48px)':'repeat(2,52px)'],['height',mobile?'104px':'112px']])important(grid,p,v);}
}
...
async function normalizeLearnedRows(){const data=await enrichment(),entries=data?.entries||{};for(const row of document.querySelectorAll('#learned-library-section .kwf-learned-row')){row.dataset.kwfUi='v3.2';important(row,'align-items','center');const main=row.querySelector('.learned-word-main'),meaning=row.querySelector('.learned-word-meaning'),status=row.querySelector('.learned-word-status');for(const el of [main,meaning,status]){important(el,'align-self','center');important(el,'justify-content','center');}const word=txt(main?.querySelector('b,strong'));if(!word)continue;const collocations=entries[word]?.collocations?.slice(0,2)||[];let host=row.querySelector('.kwf-row-collocation');if(!collocations.length){host?.remove();continue;}if(!host){host=document.createElement('div');host.className='kwf-row-collocation';row.appendChild(host);}host.replaceChildren(...collocations.map(item=>{const s=document.createElement('span');s.textContent=item;s.title=item;return s;}));}}
async function apply(){state.scheduled=false;injectPolishStyles();document.documentElement.dataset.kwfUi='v3.2';window.__KWF_UI_V3_READY__=true;hideMisplacedPlacement();normalizeCard();normalizeReviewStack();await normalizeLearnedRows();}
function schedule(){if(state.scheduled)return;state.scheduled=true;requestAnimationFrame(apply);}
const observer=new MutationObserver(schedule);
function boot(){window.__KWF_UI_V3_READY__=true;injectPolishStyles();observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','open']});addEventListener('resize',schedule,{passive:true});schedule();}
