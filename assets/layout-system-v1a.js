...
function injectPolishStyles() {
  ...
  style.textContent = `
    ...
    #study #study-card .pre-answer>div { position:absolute!important; left:var(--kwf-action-x)!important; right:var(--kwf-action-x)!important; bottom:0!important; display:grid!important; grid-template-columns:repeat(3,minmax(0,1fr))!important; gap:10px!important; width:auto!important; height:var(--kwf-action-h)!important; margin:0!important; padding:0!important; }
    #study #study-card .pre-answer>div>button { width:100%!important; height:var(--kwf-action-h)!important; min-height:var(--kwf-action-h)!important; max-height:var(--kwf-action-h)!important; margin:0!important; }
    ...
    #study #study-card .review-question>div { position:absolute!important; left:var(--kwf-action-x)!important; right:var(--kwf-action-x)!important; bottom:0!important; display:grid!important; grid-template-columns:repeat(2,minmax(0,1fr))!important; grid-template-rows:repeat(2,52px)!important; gap:8px 10px!important; width:auto!important; height:112px!important; margin:0!important; padding:0!important; }
    #study #study-card .review-question>div>button { width:100%!important; height:52px!important; min-height:52px!important; max-height:52px!important; margin:0!important; text-align:center!important; }
    ...
    #study #study-card .continue { position:absolute!important; left:var(--kwf-action-x)!important; right:var(--kwf-action-x)!important; bottom:18px!important; width:auto!important; height:var(--kwf-action-h)!important; min-height:var(--kwf-action-h)!important; max-height:var(--kwf-action-h)!important; margin:0!important; padding:0 18px!important; z-index:30!important; }
...
function normalizeCard(){
  ...
  const mobile=matchMedia('(max-width:900px)').matches, cardH='500px', wordH=mobile?'132px':'136px', actionH=mobile?'50px':'52px', actionX=mobile?'14px':'24px', contentInset=mobile?14:22, nestedActionX=`max(0px, calc(${actionX} - ${contentInset}px))`, nestedActionW=`calc(100% - (2 * ${nestedActionX}))`;
  ...
  const pre=card.querySelector('.pre-answer'); if(pre){for(const [p,v] of [['position','relative'],['flex','1 1 0'],['height','auto'],['min-height','0'],['max-height','none']])important(pre,p,v);const shortcut=pre.querySelector(':scope>.show-shortcut');if(shortcut)for(const [p,v] of [['position','static'],['align-self','center'],['transform','none'],['translate','none'],['margin-left','auto'],['margin-right','auto']])important(shortcut,p,v);const grid=pre.querySelector(':scope>div');if(grid){for(const [p,v] of [['box-sizing','border-box'],['position','absolute'],['left',nestedActionX],['right','auto'],['bottom','0'],['width',nestedActionW],['min-width',nestedActionW],['max-width',nestedActionW],['height',actionH],['display','grid'],['grid-template-columns','repeat(3,minmax(0,1fr))'],['gap','10px'],['transform','none'],['translate','none'],['margin','0'],['padding','0']])important(grid,p,v);for(const b of grid.querySelectorAll(':scope>button'))for(const [p,v] of [['box-sizing','border-box'],['display','flex'],['align-items','center'],['justify-content','center'],['width','100%'],['height',actionH],['min-height',actionH],['max-height',actionH],['margin','0'],['padding','0 14px'],['transform','none'],['translate','none']])important(b,p,v);}}
  ...
  const cont=card.querySelector('.continue'); if(cont)for(const [p,v] of [['box-sizing','border-box'],['position','absolute'],['left',actionX],['right','auto'],['bottom',mobile?'14px':'18px'],['width',`calc(100% - (2 * ${actionX}))`],['min-width',`calc(100% - (2 * ${actionX}))`],['max-width',`calc(100% - (2 * ${actionX}))`],['height',actionH],['min-height',actionH],['max-height',actionH],['margin','0'],['z-index','30'],['transform','none'],['translate','none']])important(cont,p,v);
  ...
  const question=card.querySelector('.review-question'); if(question){for(const [p,v] of [['position','relative'],['flex','1 1 0'],['height','auto'],['min-height','0'],['max-height','none'],['overflow','visible']])important(question,p,v);const shortcut=question.querySelector(':scope>.show-shortcut');if(shortcut)for(const [p,v] of [['position','static'],['align-self','center'],['transform','none'],['translate','none'],['margin-left','auto'],['margin-right','auto']])important(shortcut,p,v);const grid=question.querySelector(':scope>div');if(grid){for(const [p,v] of [['box-sizing','border-box'],['position','absolute'],['left',nestedActionX],['right','auto'],['bottom','0'],['width',nestedActionW],['min-width',nestedActionW],['max-width',nestedActionW],['display','grid'],['grid-template-columns','repeat(2,minmax(0,1fr))'],['grid-template-rows',mobile?'repeat(2,48px)':'repeat(2,52px)'],['gap','8px 10px'],['height',mobile?'104px':'112px'],['transform','none'],['translate','none'],['margin','0'],['padding','0']])important(grid,p,v);for(const b of grid.querySelectorAll(':scope>button'))for(const [p,v] of [['box-sizing','border-box'],['display','flex'],['align-items','center'],['justify-content','center'],['width','100%'],['height',mobile?'48px':'52px'],['min-height',mobile?'48px':'52px'],['max-height',mobile?'48px':'52px'],['margin','0'],['padding','0 14px'],['transform','none'],['translate','none']])important(b,p,v);}}
}
function normalizeReviewStack(){const board=document.querySelector('#review .review-board');if(!board)return;board.classList.remove('kwf-v2-overview-stack','kwf-v3-review-stack');board.classList.add('kwf-layout-review-stack');important(board,'display','flex');important(board,'flex-direction','column');important(board,'width','100%');for(const el of board.querySelectorAll('.learned-stat,.memory-profile,.memory-note,#learned-library-section')){important(el,'width','100%');important(el,'max-width','100%');important(el,'grid-column','1 / -1');}}
function scrubLegacyLayoutMarkers(){for(const el of document.querySelectorAll('[class*="kwf-v2"],[class*="kwf-v3"]')){for(const cls of [...el.classList]){if(/^kwf-v[23]/.test(cls))el.classList.remove(cls);}}}
function blockLegacyLayoutMarkers(){if(window.__KWF_LAYOUT_MARKER_BLOCKED__)return;window.__KWF_LAYOUT_MARKER_BLOCKED__=true;const add=DOMTokenList.prototype.add;DOMTokenList.prototype.add=function(...tokens){return add.apply(this,tokens.filter(token=>!/^kwf-v[23]/.test(String(token))));};}
...
async function apply(){state.scheduled=false;injectPolishStyles();document.documentElement.dataset.kwfLayoutSystem='layout-v1';document.documentElement.dataset.kwfLayoutAuthority='layout-system';window.__KWF_LAYOUT_SYSTEM_READY__=true;hideMisplacedPlacement();normalizeCard();normalizeReviewStack();await normalizeLearnedRows();scrubLegacyLayoutMarkers();}
function schedule(){if(state.scheduled)return;state.scheduled=true;requestAnimationFrame(apply);}
const observer=new MutationObserver(schedule);
function boot(){window.__KWF_LAYOUT_SYSTEM_READY__=true;blockLegacyLayoutMarkers();injectPolishStyles();observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','open']});addEventListener('resize',schedule,{passive:true});setInterval(scrubLegacyLayoutMarkers,100);schedule();}
