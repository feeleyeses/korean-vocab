...
- Specific Mobile Regression Closure: reproduced the reported `교통사고` / `고등학교` / `같다` mobile reveal paths before commit, tightened the v3 card shell into Header / Hero+Status / scrollable Knowledge / Footer regions, removed the mobile study help prompt, compacted mobile header counters and main-content review secondary buttons, and added a targeted mobile regression script for long reveal content, polysemy queue advancement, review option margins, and collapsed polysemy-library radius.
- Mobile Long-Content and Polysemy Flow Fix: kept vocabulary expansion paused, prevented revealed learning cards from clipping collocations by allowing the knowledge area to scroll only when long content overflows, stopped fake example rendering from `sense.gloss`, routed the mobile review submenu through real review/polysemy entry points, and reset polysemy learning/review state so queues advance beyond `같다`/`잡다` without carrying submitted answers into the next item.
...
- Specific Mobile Regression Follow-up: aligned the browser smoke check with the removed mobile learning-rule entry, locked mobile polysemy secondary buttons to the compact two-column height used by the consistency audit, and stabilized the Pages UI regression click after hover checks, with no vocabulary expansion.
...
- Bulk TOPIK-5/6 Expansion and Collocation Cleanup: imported 900 additional TOPIK II KRDict exact-headword Chinese matches as approved TOPIK-5/6 entries, grew the published learning vocabulary from 2081 to 2981 entries, filled 61 high-confidence manual-review `collocationZh` values with `EDITORIAL_REVIEW`, and kept missing phrase translations empty under the no-fallback rule.
- Bulk TOPIK-3/4 Expansion: extracted TOPIK II candidates from the 2662-word source PDF, imported 850 KRDict exact-headword Chinese matches as approved entries, promoted the previous 120 exact draft entries to approved, grew the published learning vocabulary from 1111 to 2081 entries, and added warning-type aggregation to the quality audit.
